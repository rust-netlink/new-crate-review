...
async fn main() -> Result<(), Error> {
    ...
    // Two connections, one for sending the request, one for receiving the async response.
    let (mut connection_1, _, mut messages) = new_connection().unwrap();
    let (connection_2, handle, _) = new_connection().unwrap();
    ...
    tokio::spawn(connection_1);

    // Start a task for receiving an async AddSa response.
    let h1 = tokio::spawn(async move {
        if let Some((message, _)) = messages.next().await {
    ...
    tokio::spawn(connection_2);

    if let Err(e) = add_upd_state(handle.clone(), &cli_args) {
        eprintln!("{}", e);
    }
...
fn add_upd_state(handle: Handle, ca: &StateAddUpdCliArgs) -> Result<(), Error> {
    ...
    // Send the request without waiting for any async response.
    req.execute_noack()?;
